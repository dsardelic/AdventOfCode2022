...
class Monkey:
    ...
    def throw_item(self, lcm):
        self.inspections += 1
        item_to_throw = self.operation(self.items.pop()) % lcm
...
def solution(input_rel_uri):
    ...
    monkeys, lcm = initialize_monkeys(monkey_data)
    do_monkey_business(monkeys, lcm)
...
def initialize_monkeys(monkey_data):
    ...
    for monkey_data_group in monkey_data:
        ...
        divisor = lines[3].strip().split("Test: divisible by ")[1]
        lcm *= int(divisor)
        test = eval("lambda x: not x % " + divisor)
        recipient_indices = [
...
def do_monkey_business(monkeys, lcm):
    for _ in range(10_000):
        for monkey in monkeys:
            for _ in range(len(monkey.items)):
                item, recipient_index = monkey.throw_item(lcm)
